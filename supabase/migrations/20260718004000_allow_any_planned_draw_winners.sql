...
BEGIN
  ...
  IF _daily_winner_user_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.profiles WHERE id = _daily_winner_user_id
  ) THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'daily_user_not_found', 'draw_date', v_date);
  END IF;

  IF _follow_winner_user_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.profiles WHERE id = _follow_winner_user_id
  ) THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'follow_user_not_found', 'draw_date', v_date);
  END IF;
...
BEGIN
  ...
  SELECT COALESCE(MAX(normal_base_reward_inmu), 10000), COALESCE(MAX(w_reward_inmu), 200000)
  ...
  IF v_daily_count = 0 AND v_follow_count = 0 AND v_planned_daily_id IS NULL AND v_planned_follow_id IS NULL THEN
  ...
  IF v_planned_follow_id IS NOT NULL THEN
    ...
    FROM public.profiles p
    WHERE p.id = v_planned_follow_id
    LIMIT 1;
